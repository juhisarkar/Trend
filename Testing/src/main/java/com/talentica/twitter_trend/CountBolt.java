...
public class CountBolt implements IRichBolt{
	//static Logger logger = Logger.getLogger(CountBolt.class);
	...
	public void writeToFile(Map<String, Long> resultMap) {
		try {
			//this.fileName= fileName+ DateTime.now().toString("yyyy-dd-M--HH-mm-ss")+".txt";
			 ...
			 fw.write("******************************************* \n");
			 fw.write("Record for This Minute Starts \n \n");
			 for(Map.Entry<String, Long>entry : resultMap.entrySet()) {
				fw.write(entry.getKey()+": "+entry.getValue()+ "\n");
				}
				fw.write("\nRecord for This Minute Ends \n");
				fw.write("******************************************* \n \n \n");
				fw.close();    
		...
		try
		{
			...
			//System.out.println("*******************" + word);
		 	if (word == null || word.trim().equals(""))
		 	{
		 		...
		 		//logger.error("Null value is passed");
		 	}
